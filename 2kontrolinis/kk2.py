...
class Admin(User):
    def __init__(self, f_name, l_name, b_year):
        super().__init__(f_name, l_name, b_year)
        self.privileges = ["Full"]

    @classmethod
    def from_string(cls, text):
        return Admin(*text.split())
...
for i in range(n):
    ...
    if input_string == "Default user":
        id_counter += 1
        if id_counter == 1:
            users[id_counter] = Admin.from_string(User.default_user_data())
        else:
        ...
        if command == "Create":
            id_counter += 1
            if id_counter == 1:
                users[id_counter] = Admin.from_string(text)
            else:
